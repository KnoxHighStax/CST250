...
namespace MineSweeperClasses
{
    ...
    public class Board
    {
        ...
        public Cell[] GetCellsFlatArray()
        {
            Cell[] flatArray = new Cell[Size * Size];
            for (int row = 0; row < Size; row++)
            {
                for (int col = 0; col < Size; col++)
                {
                    flatArray[row * Size + col] = Cells[row, col];
                }
            }
            return flatArray;
        }

        // Method to helo with deserialization
        public void SetCellsFlatArray(Cell[] flatArray)
        {
            for (int row = 0; row < Size; row++)
            {
                for (int col = 0; col < Size; col++)
                {
                    Cells[row, col] = flatArray[row * Size + col];
                }
            }
        }
        ...
        public void FloodFill(int row, int col)
        {
            ...
            if (row < 0 || row >= Size || col < 0 || col >= Size || Cells[row, col].IsVisited)
                return;
